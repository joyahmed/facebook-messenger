...
interface CredentialFormProps {
  ...
  validate: Validate;
  setErrors: SetErrors;
  handleSubmitForm: (submitCallback: SubmitCallback<AuthForm>) => (e: FormEvent<Element>) => void;
...
const CredentialForm = ({
  ...
  const { inputItems, isLoading, toggleVariant } = useAuthItems(
    form,
    validate,
    setErrors
  );
  ...
  return (
    <form className='space-y-3' onSubmit={handleSubmitForm(onFormSubmit)}>
      {inputItems.map(item => (
        <div
          key={item.id}
          className={`${
